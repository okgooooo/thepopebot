...
export async function triggerRoleManually(roleId) {
  ...
  const { runClusterRole } = await import('./execute.js');
  const result = await runClusterRole(roleId);
  return result;
}
...
export async function getSessionLog(clusterId, roleShortId, sessionName, file) {
  ...
  const allowed = { stdout: 'stdout.jsonl', stderr: 'stderr.txt', prompt: 'prompt.md' };
